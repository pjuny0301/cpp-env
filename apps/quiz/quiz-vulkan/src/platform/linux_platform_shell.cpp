...
class linux_platform_shell final : public platform_shell {
public:
    bool create(const platform_shell_config& config) override
    {
        state_.client_size = configured_client_size(config);
        std::cout << "linux platform shell placeholder: " << config.title << " "
...
{
    return std::make_unique<linux_platform_shell>();
}
