...
- `common/`: shared Codex instructions, agent handoff, token-budget, and MCP material used across projects.
- `quiz/`: quiz-specific Codex context. It keeps `big_plan.md`, `구현/`, and `일자별요구사항/`.
- For quiz-specific work, start at `quiz/AGENTS.md`.
- For reusable Codex/tooling work, start at `common/AGENTS.md`.
- New project Codex context should follow this shape: `codex/<project>/big_plan.md` for the top plan, `codex/<project>/구현/NN.md` for per-requirement implementation plans, and `codex/<project>/일자별요구사항/YYYY-MM-DD/` for daily records and evidence.
- In project plans, requirement numbers are trace IDs. The actual execution order should be dependency-based and recorded in `big_plan.md`.
...
2. Run `build/external/tools/bootstrap/setup_codex_env.ps1`.
3. Keep durable runtime and tool paths ASCII-only under `C:\codex-local` and `C:\dev`.
4. When coordinating multiple local agents, write the shared task once in `codex/common/agent/shared_command.json` and tell sub-agents to read that file first.
5. Unless the user explicitly asks otherwise, keep Codex-common work inside `codex/` and reusable build/tool setup inside `build/external/`, and treat `apps/` as product workspaces.
6. When working inside `codex/common`, `codex/quiz`, `build/external`, or `apps/quiz`, prefer that subtree's local `AGENTS.md` before broad repo files.

## Token Budget

- Prefer compact metadata over large artifacts.
- Read `codex/common/agent/token_budget.json` before broad repo scans.
- For generated plan bundles under `codex/quiz/일자별요구사항/*/generated/`, read `bundle_manifest.json` and `prompt_overrides.json` before `render_spec.json`, `prompts.json`, or `plan_summary.docx`.
