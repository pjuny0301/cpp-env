...
- Latest integrated baseline includes architecture boundary locks, text line-break/glyph cache/font-resolution/readiness/atlas-page/upload-policy diagnostics, image sampler/cache/upload queue/retry/lifetime eviction and decoder diagnostics, asset pack index/lookup/validation/manifest/runtime resolver validation, Vulkan frame/pipeline/command-buffer-submit lifecycle/resource diagnostics, input routing/action/text-edit/IME preedit/gesture-cancel policy diagnostics, and procedural audio mixer event unification.
