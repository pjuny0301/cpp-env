...
| 82 | 0. 작업 기준 고정<br>4. Vulkan 플레이어 재작성 | vulkan 렌더러로 구현 - 순수 c++ | [82](구현/82.md); [quiz-vulkan](quiz-vulkan/구현/82.md) | `src/app/app_demo.*`, `src/app/app.cpp`, `src/app/main.cpp`, `src/platform/windows_platform_shell.cpp`, `tests/app/app_demo_tests.cpp`, `src/render/render_draw_list.h`, `src/render/vulkan/*`, `src/render/text/*`, `src/render/image/*`, renderer가 `core/scene`, `core/ui`, `core/domain` 없이 render-only draw list 소비, CPU fallback framebuffer present path, Vulkan frame sync/pipeline/command-buffer-submit lifecycle/resource binding/resource registry diagnostics | 부분 구현 | swapchain/text backend 추가 전까지 app snapshot render path와 입력 route 확대 |
...
- Text/Image engines: `apps/quiz/quiz-vulkan/src/render/text/*`, `apps/quiz/quiz-vulkan/src/render/image/*`, `apps/quiz/quiz-vulkan/tests/render/fake_text_engine_tests.cpp`, `apps/quiz/quiz-vulkan/tests/render/fake_image_texture_cache_tests.cpp`, line-break/glyph cache/font-resolution/atlas-page upload-policy and sampler/cache/upload queue/lifetime eviction/decoder format diagnostics
- Asset system: `apps/quiz/quiz-vulkan/src/assets/*`, `apps/quiz/quiz-vulkan/tests/assets/*`, asset manifest/runtime resolver/pack index/pack lookup/pack validation diagnostics
- Input/Audio engines: `apps/quiz/quiz-vulkan/src/core/input/*`, `apps/quiz/quiz-vulkan/tests/input/*`, input routing/action/text-edit/IME preedit/gesture-cancel policy diagnostics, `apps/quiz/quiz-vulkan/src/audio/*`, `apps/quiz/quiz-vulkan/tests/audio/*`
