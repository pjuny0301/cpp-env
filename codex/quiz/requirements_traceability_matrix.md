...
- `16dfb57`: architecture boundary test로 domain/app/input/audio와 render/ui/layout 직접 의존을 잠금.
- `ea52f0e`: fake image pipeline diagnostics 추가.
- `684e3d7`: asset path policy contract를 aggregate interface lock에 반영.
- `9b62615`: Vulkan frame sync diagnostics 추가.
- `eab4055`: IME composition state contract 추가.
- `d8215dc`: fake text atlas placement diagnostics 추가.
...
| 82 | 0. 작업 기준 고정<br>4. Vulkan 플레이어 재작성 | vulkan 렌더러로 구현 - 순수 c++ | [82](구현/82.md); [quiz-vulkan](quiz-vulkan/구현/82.md) | `src/app/app_demo.*`, `src/app/app.cpp`, `src/app/main.cpp`, `src/platform/windows_platform_shell.cpp`, `tests/app/app_demo_tests.cpp`, `src/render/render_draw_list.h`, `src/render/vulkan/*`, `src/render/text/*`, `src/render/image/*`, renderer가 `core/scene`, `core/ui`, `core/domain` 없이 render-only draw list 소비, CPU fallback framebuffer present path, Vulkan frame sync diagnostics | 부분 구현 | swapchain/text backend 추가 전까지 app snapshot render path와 입력 route 확대 |
